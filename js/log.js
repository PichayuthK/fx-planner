(function () {
  ...
  ForexPlan.renderLogs = function () {
    ...
    tbody.innerHTML = logs
      .sort((a, b) => {
        // `id` is a Unix timestamp (ms); sort newest first
  ...
  ForexPlan.updateComparison = function () {
    ...
    try {
      ...
      const t = ForexPlan.t;
      el.className = `banner ${isOnTrack ? "on-track" : "in-progress"}`;
      el.innerHTML = isOnTrack
        ? `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg> ${t("onTrack")} — $${fn(actualCapital)} ${t("vsProjected")} ~$${fn(projectedForPeriod)} (${t("exceedProjected")} +$${fn(diff)})`
        : `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> ${t("inProgress")} — $${fn(actualCapital)} ${t("vsProjected")} ~$${fn(projectedForPeriod)}`;
    } catch {
      el.className = "banner in-progress";
